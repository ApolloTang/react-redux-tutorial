...
const externalRender = function(model) {
    render(<Root model={model}/>, reactContainer);
};
externalRender(model_external);

const externalButton = document.createElement('button')
externalButton.innerText = 'Add model via props';
document.getElementsByTagName('body')[0].appendChild( externalButton );

const handle_addModelItem = function() {
    const _id = Date.now().toString();

    // const model_prev = this.state.model;
    const model_prev = model_external;
    const newItem = {
        id: _id,
        value: _id
    };
